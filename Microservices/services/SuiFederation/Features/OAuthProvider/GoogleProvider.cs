...
public class GoogleProvider : IService, IOauthProvider
{
    ...
    public GoogleProvider(EnokiService enokiService, OAuthRequestCollection oAuthRequestCollection, Configuration configuration, HttpClientService httpClientService)
    {
    ...
    public async Task<string> GetAuthorizationUrl(long gamerTag)
    {
        ...
        var redirectUri = $"https://api.beamable.com/basic/{ProjectInfo.Cid}.{ProjectInfo.Pid}.micro_{SuiFederationSettings.MicroserviceName}/OAuthCallback";
    ...
    public async Task<EnokiAddress?> Resolve(long gamerTag, string challenge, string solution)
    {
        try
        {
            ...
            if (!challengeUri.ValidateChallengeUri(requestData))
            {
